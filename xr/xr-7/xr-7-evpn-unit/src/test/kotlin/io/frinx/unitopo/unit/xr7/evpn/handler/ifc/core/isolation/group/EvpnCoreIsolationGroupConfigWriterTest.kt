...
import org.opendaylight.yang.gen.v1.http.cisco.com.ns.yang.cisco.ios.xr.l2vpn.cfg.rev190405.EvpnCoreGroupIdRange
...
class EvpnCoreIsolationGroupConfigWriterTest : AbstractNetconfHandlerTest() {
    @Mock
    private lateinit var writeContext: WriteContext

    private lateinit var underlayAccess: UnderlayAccess

    private lateinit var target: EvpnCoreIsolationGroupConfigWriter

    companion object {
        val NC_HELPER = NetconfAccessHelper("/data_nodes.xml")
        val IID_CIG_CONFIG = IIDs.EV_INTERFACES
    ...
    fun setUp() {
        ...
        target = Mockito.spy(EvpnCoreIsolationGroupConfigWriter(underlayAccess))
    }
    ...
    fun testWriteCurrentAttributes() {
        ...
        val expected = EvpnInterfaceBuilder().apply {
            ...
            this.evpnCoreIsolationGroup = EvpnCoreGroupIdRange(100)
        }.build()
        ...
        target.writeCurrentAttributes(IID_CIG_CONFIG, data, writeContext)
    ...
    fun testUpdateCurrentAttributes() {
        val dataBefore = ConfigBuilder().apply {
            this.id = 100
        }.build()
        ...
        val expected = EvpnInterfaceBuilder().apply {
            this.interfaceName = InterfaceName("Bundle-Ether65535")
            this.evpnCoreIsolationGroup = EvpnCoreGroupIdRange(200)
        }.build()
        ...
        target.updateCurrentAttributes(IID_CIG_CONFIG, dataBefore, dataAfter, writeContext)
