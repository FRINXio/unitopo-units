...
class EvpnReaderTest : AbstractNetconfHandlerTest() {
    @Mock
    private lateinit var readContext: ReadContext

    private lateinit var underlayAccess: UnderlayAccess

    private lateinit var target: EvpnReader
    ...
    fun setUp() {
        ...
        target = Mockito.spy(EvpnReader(underlayAccess))
    }

    @Test
    fun testReadCurrentAttributes() {
        val builder = EvpnBuilder()
        target.readCurrentAttributes(IIDs.EVPN, builder, readContext)
        Assert.assertTrue(builder.config.isEnabled)
    }
}